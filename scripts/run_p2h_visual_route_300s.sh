...
RUN_ID="p2h_visual_route_300s_${STAMP}"
OUT="test-log/${STAMP}_p2h_visual_route_300s_runtime.txt"
...
python3 scripts/fuel_route_rationality_recorder.py --duration 300 --run-id "$RUN_ID" --output-root reports/p2h_metrics --progress-interval 30 | tee "$OUT.recorder"
python3 scripts/analyze_route_rationality.py --metrics-dir "reports/p2h_metrics/$RUN_ID" | tee "$OUT.analysis" || true
echo "300s P2H visual route demo finished. Press Enter to cleanup, or inspect RViz now."
read -r _ || true
