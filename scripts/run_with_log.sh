...
if [ -f "$FUEL_WS/scripts/generate_full_run_log.py" ]; then
  FULL_LOG_OUTPUT="$(python3 "$FUEL_WS/scripts/generate_full_run_log.py" \
    --task-name "$TASK_NAME" \
    --command "$COMMAND_STRING" \
    --exit-code "$EXIT_CODE" \
    --raw-log "$LOG_FILE" \
    ...
    --workspace "$FUEL_WS" 2>&1)"
  FULL_LOG_STATUS=$?
  echo "$FULL_LOG_OUTPUT"
  if [ "$FULL_LOG_STATUS" -eq 0 ]; then
    FULL_LOG_PATH="$(printf '%s\n' "$FULL_LOG_OUTPUT" | awk -F= '/^FULL_RUN_LOG_CREATED=/{print $2}' | tail -1)"
    {
      echo
      echo "## Full Run Log"
      echo
      echo '```text'
      echo "FULL_RUN_LOG_CREATED=$FULL_LOG_PATH"
      echo '```'
    } >> "$LOG_FILE"
  else
    {
      ...
      echo
      echo '```text'
      echo "FULL_RUN_LOG_FAILED=$FULL_LOG_OUTPUT"
      echo '```'
    } >> "$LOG_FILE"
  fi
else
  {
    ...
    echo
    echo '```text'
    echo "FULL_RUN_LOG_FAILED=full_log_generator_missing"
    echo '```'
  } >> "$LOG_FILE"
fi
