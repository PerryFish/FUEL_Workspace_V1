...
RUN_ID="p2i_${MODE}_${STAMP}"
OUT="test-log/${STAMP}_p2i_${MODE}_runtime.txt"
LAUNCH_PID=""
TF_PID=""
cleanup() {
...
if [ "$DURATION" = "0" ]; then
  python3 scripts/fuel_route_rationality_recorder.py \
    --duration 0 \
    --run-id "$RUN_ID" \
    --output-root reports/p2i_metrics \
    --progress-interval 30 | tee "$OUT.recorder" &
  REC_PID=$!
  echo "P2I_FULL_HEADLESS_ROUTE_RUNNING run_id=$RUN_ID"
  echo "Press Enter to stop and cleanup, or use Ctrl+C."
  read -r _ || true
...
echo "P2I_${MODE}_DONE run_id=$RUN_ID" | tee -a "$OUT"
exit "$REC"
