...
RUN_ID="p2g_visual_coverage_300s_${STAMP}"
OUT="test-log/${STAMP}_p2g_visual_coverage_300s_runtime.txt"
...
python3 scripts/fuel_frontier_reachability_recorder.py --duration 300 --run-id "$RUN_ID" --output-root reports/p2g_metrics --progress-interval 30 | tee "$OUT.recorder"
...
echo "300s P2G visual coverage demo finished. Press Enter to cleanup, or inspect RViz now."
read -r _ || true
