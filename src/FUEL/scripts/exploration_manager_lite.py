...
class ExplorationManagerLite(Node):
    def __init__(self):
        ...
        self.escape_cooldown_sec = float(self.declare_parameter("escape_cooldown_sec", 25.0).value)
        self.enable_frontier_point_escape = bool(self.declare_parameter("enable_frontier_point_escape", False).value)
    ...
    def _switch_to_escape_goal(self, reason: str) -> bool:
        if not self._rate_limit_allows_switch(self._now()):
